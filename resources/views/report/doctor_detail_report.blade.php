                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    <tbody>
                                        @php
                                            ...
                                            $total_commission = 0;
                                        @endphp
                                        @if (!empty($doctor_detail_search))
                                            @foreach ($doctor_detail_search as $doctor)
                                                <tr>
                                                    <td>{{ $no }}</td>
                                                    <td>{{ $doctors->name }}</td>
                                                    <td>{{ $doctor->customer_name }}</td>
                                                    <td>{{ $doctor->invoice_no }}</td>
                                                    <td>{{ $doctor->created_at->format('d M Y') }}</td>
                                                    @php
                                                        $totallabsale = 0;
                                                    @endphp
                                                    @foreach ($sells as $sell)
                                                        @if ($sell->invoiceid == $doctor->id)
                                                            @php $totallabsale += $sell->sale_price; @endphp
                                                        @endif
                                                    @endforeach
                                                    <td class="text-right">
                                                        {{ $commission = $doctor->doctor_commission - $totallabsale }}
                                                    </td>
                                                </tr>
                                                @php
                                                    $no++;
                                        ...
                                        @else
                                            @foreach ($invoices as $invoice)
                                                <tr>
                                                    ...
                                                    @endphp
                                                    @foreach ($sells as $sell)
                                                        @if ($sell->invoiceid == $invoice->id)
                                                            @php $totallabsale += $sell->sale_price; @endphp
                                                        @endif
                                                    @endforeach
                                                    <td class="text-right">
                                                        {{ number_format($commission = $invoice->doctor_commission - $totallabsale) }}
                                                    </td>
                                                </tr>
                                                @php
                                                    $total_commission += $commission;
